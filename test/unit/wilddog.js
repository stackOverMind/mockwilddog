...
var Wilddog = require('../../').MockWilddog;

describe('MockWilddog', function () {

  var ref, spy;
  beforeEach(function () {
    ref = new Wilddog().child('data');
  ...
  describe('Server Timestamps', function () {
    ...
    it('parses server timestamps', function () {
      ref.set(Wilddog.ServerValue.TIMESTAMP);
      ref.flush();
    ...
    it('parses server timestamps in child data', function () {
      ...
      ref.set({
        foo: Wilddog.ServerValue.TIMESTAMP
      });
      ref.flush();
    ...
    it('parses server timestamps in priorities', function(){
      ref.setPriority(Wilddog.ServerValue.TIMESTAMP);
      ref.flush();
      expect(ref).to.have.property('priority', new Date().getTime());
    });

    describe('Wilddog#setClock', function () {

      afterEach(Wilddog.restoreClock);

      it('sets a timestamp factory function', function () {
        var customClock = sinon.stub().returns(10);
        Wilddog.setClock(customClock);
        ref.set(Wilddog.ServerValue.TIMESTAMP);
        ref.flush();
    ...
    describe('#restoreClock', function () {
      ...
      it('restores the normal clock', function () {
        Wilddog.setClock(spy);
        Wilddog.restoreClock();
        ref.set(Wilddog.ServerValue.TIMESTAMP);
        ref.flush();
  ...
  describe('#set', function () {
    ...
    it('should fire child_added events with correct prevChildName', function () {
      ref = new Wilddog('Empty://', null).autoFlush();
      ref.set({
        alpha: {
          '.priority': 200,
          foo: 'alpha'
