...
WilddogAuth.prototype.changeAuthState = function (userData) {
...
WilddogAuth.prototype.getEmailUser = function (email) {
...
Object.keys(authMethods)
  .forEach(function (method) {
    ...
    WilddogAuth.prototype[method] = function () {
      this._authEvent(method, arguments[callbackIndex]);
    };
  });

WilddogAuth.prototype.auth = function (token, callback) {
  console.warn('FIREBASE WARNING: WilddogRef.auth() being deprecated. Please use WilddogRef.authWithCustomToken() instead.');
  this._authEvent('auth', callback);
};

WilddogAuth.prototype._authEvent = function (method, callback) {
...
WilddogAuth.prototype._triggerAuthEvent = function () {
...
WilddogAuth.prototype.getAuth = function () {
  return this._auth.userData;
};

WilddogAuth.prototype.onAuth = function (onComplete, context) {
...
WilddogAuth.prototype.offAuth = function (onComplete, context) {
...
WilddogAuth.prototype.unauth = function () {
...
WilddogAuth.prototype.createUser = function (credentials, onComplete) {
...
WilddogAuth.prototype.changeEmail = function (credentials, onComplete) {
...
WilddogAuth.prototype.changePassword = function (credentials, onComplete) {
...
WilddogAuth.prototype.removeUser = function (credentials, onComplete) {
...
WilddogAuth.prototype.resetPassword = function (credentials, onComplete) {
...
WilddogAuth.prototype._nextUid = function () {
  return 'simplelogin:' + (this._auth.uidCounter++);
};

WilddogAuth.prototype._validateNewEmail = function (credentials) {
...
WilddogAuth.prototype._validateExistingEmail = function (credentials) {
  if (!this._auth.users.hasOwnProperty(credentials.email)) {
...
WilddogAuth.prototype._validPass = function (object, name) {
  var err = null;
...
function validateObject (object, method, position) {
  if (!_.isObject(object)) {
    throw new Error(format(
      'Wilddog.%s failed: %s argument must be a valid object.',
      method,
      position
    ));
  }
...
function validateArgument (method, object, position, name, type) {
  if (!object.hasOwnProperty(name) || typeof object[name] !== type) {
    throw new Error(format(
      'Wilddog.%s failed: %s argument must contain the key "%s" with type "%s"',
      method,
      position,
      name,
      type
